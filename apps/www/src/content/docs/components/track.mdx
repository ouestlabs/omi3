...
<Step>Adjust import paths as needed for your repository layout.</Step>
...
- Prefer providing either `trackId` (store mode) or `track` (controlled mode). Both may be omitted for some internal uses, but passing both is not supported.
- Cover images are taken from `track.artwork` or `track.images[0]`.
- Live tracks show a "Live" badge and the duration is hidden.
...
- When `sortable` is true, reordering will call the store `setQueue` only if the list is not filtered and not controlled by the `tracks` prop.
- Sortable grid/list rendering uses the registry's `SortableList`, `SortableItem`, and `SortableDragHandle` components.
...
- `AudioProvider` (or the project's audio store) is required for store mode to work correctly.
- Live tracks hide duration and show a prominent badge.
- When building UI around these components, keep `sortable` off while filtering to avoid confusing reordering behavior.
- If you localize strings (e.g., empty labels), pass `emptyLabel` / `emptyDescription` to the list.