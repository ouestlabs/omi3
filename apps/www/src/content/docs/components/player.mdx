...
- `AudioProvider` (or the project's audio store) is required for player components to work correctly.
...
- Previous button restarts the track if played more than 3 seconds, otherwise goes to the previous track.
- Live streams disable seeking, rewind, and fast-forward controls.