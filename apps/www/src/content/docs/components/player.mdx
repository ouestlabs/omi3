...
<Callout variant="info">
  <strong>AudioProvider Required:</strong> Wrap your app with{" "}
  <code>AudioProvider</code> at the root level. This provides the audio context
...
```tsx
<AudioPlayer>{/* Your audio controls */}</AudioPlayer>
```
...
<Callout variant="info">
  <strong>Keyboard Shortcuts:</strong> The <code>AudioPlayerPlay</code>{" "}
...
<Callout variant="info">
  ...
  - **Component Architecture:** All audio components are self-contained and read
    state directly from the global store. The audio player uses the HTML5 audio
    element under the hood.
  - **AudioProvider Role:** The `AudioProvider` component manages the audio
    element lifecycle, handles playback events, manages error retries (up to 3
    retries with 1 second delay), preloads next tracks, and synchronizes audio
    state with the Zustand store. It's required for all audio components to
    function correctly.
  - **Previous Button Behavior:** The previous button restarts the track if it has
    been played for more than 3 seconds, otherwise it navigates to the previous
    track in the queue.
