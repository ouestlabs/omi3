---
title: Audio Queue
description: Composable queue management components for audio players with shuffle, repeat, and track selection.
featured: true
component: true
---

A comprehensive queue management system for audio players. Includes shuffle, repeat modes, track selection, search, and drag-and-drop reordering.
...
```bash
npx shadcn@latest add button dialog dropdown-menu input item scroll-area toggle tooltip
```

<Step>Install the custom components from this registry:</Step>

```bash
npx shadcn@latest add @audio/empty
```
...
- All queue components are self-contained and read state directly from the global store
- `AudioProvider` must be present in the component tree for components to work
- The `AudioQueue` component includes built-in search functionality
- Track reordering is disabled when search is active
- The queue dialog automatically closes when a track is selected
- Queue components are optimized to prevent unnecessary re-renders