...
<Callout variant="info">
  <strong>Search and Drag-and-Drop:</strong> Search functionality filters tracks
  by title or artist. Track reordering is automatically disabled when search is
  active to prevent confusion. The queue dialog automatically closes when a
  track is selected.
</Callout>
...
<Callout variant="info">
  <strong>State Persistence:</strong> The shuffle and repeat mode states are
  persisted to localStorage and will be restored when the app reloads.
</Callout>
...
<Callout variant="info">
  <strong>Insert Mode Behavior:</strong> The insert mode determines where new
  tracks are added to the queue when using <code>addToQueue</code>. This setting
...
<Callout variant="info">
  <strong>AudioProvider Requirement:</strong> <code>AudioProvider</code> (or the
  ...
  manages the audio playback lifecycle and synchronizes queue state with the
  audio element.
</Callout>
