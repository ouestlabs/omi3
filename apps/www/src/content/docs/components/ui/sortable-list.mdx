...
- **Custom Component:** This component is not part of the default shadcn/ui
  registry. It's a custom component built specifically for audio/ui to enable
  drag-and-drop reordering of tracks in the queue.
- **Built with dnd-kit:** This component is built on top of <code>@dnd-kit</code>,
  which provides full keyboard navigation support. Users can reorder items using
  arrow keys and keyboard shortcuts. See the{" "}
  <a
  ...
  for more details.
  - **Drag Handle:** The <code>SortableDragHandle</code> component uses shadcn/ui's{" "}
    <code>Button</code> component and provides a pre-configured drag handle with proper
    accessibility attributes.
  - **Item IDs:** Each item must have a unique <code>id</code> property. The{" "}
    <code>id</code> type must match between the item object and the <code>
      SortableItem
    </code> component.
  - **Usage in Audio Components:** This component is used by{" "}
    <code>AudioTrackList</code> to enable drag-and-drop reordering of tracks in
    the queue.
</Callout>
