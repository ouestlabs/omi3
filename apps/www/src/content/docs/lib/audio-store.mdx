...
links:
  doc: https://zustand.docs.pmnd.rs/getting-started/introduction
  api: https://zustand.docs.pmnd.rs/apis/create
---
...
<Callout variant="info">
  <strong>Performance Best Practices:</strong> Use granular selectors for better
...
<Callout variant="warning">
  <strong>SSR Considerations:</strong> The store uses <code>canUseDOM()</code>{" "}
  internally to handle server-side rendering. When accessing the store in SSR
  contexts, ensure you check for DOM availability before calling store methods
...
<Callout variant="info">
  ...
  - Prefer selector subscriptions for performance (subscribe to specific slices
    of state)
  - Async actions (<code>play</code>, <code>next</code>,{" "}
    <code>setCurrentTrack</code>) wait for audio loading
  ...
  - <code>setQueueAndPlay</code> and similar actions trigger audio loading via the
    [Audio library](/docs/lib/audio)
  - Persistence is automatic; no manual save required
  - Direct store access via <code>getState()</code> is useful for non-React code
    or imperative operations
</Callout>

## Changelog

### 2025-12-02 History removal and live stream improvements

- **Removed**: `history` state and `addToHistory` action (feature deprecated)
- **Added**: `setPlaybackRate()` now prevents rate changes for live streams
- **Fixed**: Playback rate automatically resets to 1.0 when loading live streams
- **Improved**: Live stream detection integrated into playback rate controls
