...
import { buttonVariants } from "audio-ui/components/button";
import { Spinner } from "audio-ui/components/spinner";
...
type ToastIconComponent = React.ComponentType<{ className?: string }>;
...
function getToastIcon(type?: string | null): ToastIconComponent | null {
  if (!type) {
    return null;
  }
  if (type in TOAST_ICONS) {
...
function ToastList({ position = "bottom-right" }: { position: ToastPosition }) {
  ...
  return (
    <Toast.Portal data-slot="toast-portal">
      ...
      >
        {toasts.map((toast) => {
          const Icon = getToastIcon(toast.type as string | null);
          const swipeDirection = getSwipeDirection(position, isTop);
            ...
            >
              <Toast.Content className="flex items-center justify-between gap-1.5 overflow-hidden text-sm transition-opacity duration-250 data-behind:pointer-events-none data-expanded:pointer-events-auto data-behind:opacity-0 data-expanded:opacity-100">
                <div className="flex gap-2">
                  {Icon && (
                    <div
                      className="mt-.5 [&>svg]:h-lh [&>svg]:w-4 [&_svg]:pointer-events-none [&_svg]:shrink-0"
